...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [Kaif-MD-MINI] ${new Date().toISOString()}: ${message}`);
}
...
async function arslanPair(number, res = null) {
    ...
    try {
        ...
        arslanLog(`Kaif-MD-MINI Pair error: ${err.message}`, 'error');
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 Kaif-MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
